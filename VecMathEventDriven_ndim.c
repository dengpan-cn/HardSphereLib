...
#define cmpEtimeLarge(eTime, ith, jth) (eTime[ith] > eTime[jth])
#define SwapHeap(minHeap, ith, jth)                \
    {                                              \
        double etime = minHeap->eTime[jth];        \
        minHeap->eTime[jth] = minHeap->eTime[ith]; \
        minHeap->eTime[ith] = etime;               \
        int aidx = minHeap->eIdx[jth];             \
        minHeap->eIdx[jth] = minHeap->eIdx[ith];   \
        minHeap->eIdx[ith] = aidx;                 \
    }

void AdjustMinEventHeapUp(MinEventHeap *minHeap, int heapIdx) {
    int aidx = minHeap->eIdx[heapIdx];
    while (cmpEtimeLarge(minHeap->eTime, heapIdx / 2, heapIdx)) {
        int jthIdx = minHeap->eIdx[heapIdx / 2];
        SwapHeap(minHeap, heapIdx / 2, heapIdx);
        minHeap->atomTag2HeapIdx[jthIdx] = heapIdx;
        heapIdx /= 2;
    }
    minHeap->atomTag2HeapIdx[aidx] = heapIdx;
}
void AdjusMinEventHeapDown(MinEventHeap *minHeap, int heapIdx) {
    int aidx = minHeap->eIdx[heapIdx];
    while (heapIdx * 2 <= minHeap->count) {
        int jth = heapIdx * 2;
        if (cmpEtimeLarge(minHeap->eTime, jth, jth + 1))
            jth = jth + 1;
        if (!cmpEtimeLarge(minHeap->eTime, heapIdx, jth))
            break;

        int jthIdx = minHeap->eIdx[jth];
        SwapHeap(minHeap, jth, heapIdx);
        minHeap->atomTag2HeapIdx[jthIdx] = heapIdx;
...
void replaceMinEventHeap(MinEventHeap *minHeap, Event *data) {
    int ithMap = minHeap->atomTag2HeapIdx[data->atomIdx];
    minHeap->eIdx[ithMap] = data->atomIdx;
    minHeap->eTime[ithMap] = data->eTime;
    if (cmpEtimeLarge(minHeap->eTime, ithMap / 2, ithMap)) {
        AdjustMinEventHeapUp(minHeap, ithMap);
    } else {
        AdjusMinEventHeapDown(minHeap, ithMap);
    }
}
void clearMinEventHeap(MinEventHeap *minHeap, int nAtom) {
    if (minHeap->MaxSize >= nAtom) {
        minHeap->count = nAtom;

        minHeap->eTime[0] = -DBL_INF;
        minHeap->eTime[nAtom + 1] = DBL_INF;
        minHeap->eIdx[0] = minHeap->eIdx[nAtom + 1] = -1;
    } else {
        minHeap->MaxSize = nAtom;
        minHeap->count = nAtom;

        minHeap->eTime =
            (double *)realloc(minHeap->eTime, (nAtom + 2) * sizeof(double));
        minHeap->eIdx = (int *)realloc(minHeap->eIdx, (nAtom + 2) * sizeof(int));
        minHeap->data = (Event *)realloc(minHeap->data, nAtom * sizeof(Event));
        minHeap->atomTag2HeapIdx =
            (int *)realloc(minHeap->atomTag2HeapIdx, nAtom * sizeof(int));

        minHeap->eTime[0] = -DBL_INF * 2.0;
        minHeap->eTime[nAtom + 1] = DBL_INF * 2.0;
        minHeap->eIdx[0] = minHeap->eIdx[nAtom + 1] = -1;
    }

    for (int ith = 1; ith <= nAtom; ith++) {
        minHeap->eIdx[ith] = ith - 1;
        minHeap->eTime[ith] = DBL_INF;

        minHeap->atomTag2HeapIdx[ith - 1] = ith;
    }
    // for (int iatom = 0; iatom < nAtom; iatom++) {
    //   minHeap->data[iatom].atomIdx = iatom;
    //   minHeap->data[iatom].ePartner = transPartner;
    //   minHeap->data[iatom].eTime = DBL_INF;
    //   minHeap->data[iatom].eventType = NoneType;
    // }
}
#undef SwapHeap
#undef cmpEtimeLarge
...
void heapSortInc(int *arr, int len) {
    ...
    // 将数组初始化为大顶堆
    for (int ith = len / 2 - 1; ith >= 0; ith--)
        max_heapify(arr, ith, len - 1);
