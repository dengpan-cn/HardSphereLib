...
#define dumpFileRevNum 2         // 0: SS 3d; 1: SS; 2: HS;
...
#define BitSet(bitArr, whichBit) \
    ((bitArr)[BitSlot(whichBit)] |= BitMask(whichBit))
#define BitClear(bitArr, whichBit) \
    ((bitArr)[BitSlot(whichBit)] &= ~BitMask(whichBit))
#define BitTest(bitArr, whichBit) \
    ((bitArr)[BitSlot(whichBit)] & BitMask(whichBit))
...
#define vIdx2lIdx(vidx, nbin) \
    ((vidx[2] * nbin[1] + vidx[1]) * nbin[0] + vidx[0])
...
#define vIdx2lIdx(vidx, nbin) (vidx[1] * nbin[0] + vidx[0])
...
//({                                                                         \
//    int mkVar(spaceIdx2voigt, min) = cpuMin((alpha), (beta));              \
//    int mkVar(spaceIdx2voigt, max) = cpuMax((alpha), (alpha));             \
//    ((mkVar(spaceIdx2voigt, min)) +                                        \
//     (mkVar(spaceIdx2voigt, max)) * (mkVar(spaceIdx2voigt, max) + 1) / 2); \
//})
...
#define vIdx2lIdx(vidx, nbin)                                     \
    ({                                                            \
        int mkVar(vIdx2lIdx, lidx) = 0;                           \
        for (int idim = DIM - 1; idim >= 0; idim--) {             \
            mkVar(vIdx2lIdx, lidx) =                              \
                mkVar(vIdx2lIdx, lidx) * nbin[idim] + vidx[idim]; \
        }                                                         \
        mkVar(vIdx2lIdx, lidx);                                   \
    })
#define lidx2vidx(lidx, nbin, vidx)                                       \
    {                                                                     \
        int mkVar(lidx2vidx, lidx) = lidx;                                \
        for (int idim = 0; idim < DIM; idim++) {                          \
            vidx[idim] = mkVar(lidx2vidx, lidx) % nbin[idim];             \
            mkVar(lidx2vidx, lidx) = mkVar(lidx2vidx, lidx) / nbin[idim]; \
        }                                                                 \
    }
